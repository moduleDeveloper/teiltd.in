...
const proofs = [
  ...
  },
  {
    ...
    desc: "475+ live HNI women members. Member directory, notices, events, gallery, sponsor visibility and community engagement.",
    badge: "475+ Live HNI Women",
...
const Proof = () => {
  ...
  const stepImage = (cardIndex: number, dir: "next" | "prev") => {
    ...
    if ((dir === "prev" && current === 0) || (dir === "next" && current === total - 1)) return;
    ...
    window.setTimeout(() => {
      setActiveIndexes((prev) =>
        prev.map((v, i) => {
          ...
          return dir === "next" ? Math.min(v + 1, total - 1) : Math.max(v - 1, 0);
        })
      );
    }, 260);
  ...
  return (
    <section ref={ref} id="proof" className="py-24 bg-gradient-hero relative overflow-hidden">
      ...
      <div className="container">
        ...
        <div className="relative mt-14 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {proofs.map((p, i) => (
            (() => {
